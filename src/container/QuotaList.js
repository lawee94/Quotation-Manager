...
class QuotaList extends Component {
    ...
    deleteMaterial = () => {
        ...
        let updatedState = this.state.quotation.filter(res =>  res.key !== this.state.key )
    ...
    showModalHandler = (list) => {
        this.setState({ edit: true, key: list.key })
    }
    ...
    viewQuotation = (list) => {
        ...
        this.setState({ key: list.key})
        let index = this.state.quotation.findIndex( res => res.key === localStorage.getItem('key') )
    ...
    render(){
        let lists = this.state.quotation.map((list, index) => {
            return <tr style={{cursor: 'pointer'}} key ={index } >
                        <td>{index + 1}</td>
                        <td  onClick={() => this.viewQuotation(list)}>{list.value.quotationName}</td>
                        <td>{list.value.quotationType}</td>
                        <td>{list.value.created}</td>
                        <td>
                            <button 
                                className="btn btn-danger" 
                                onClick={() => this.showModalHandler(list )}>Delete
                            </button>
                        </td>
                    </tr>
        })
            ...
            <div>
                <div className="col-lg-12 mt-4">
                    ...
                    <div style={{float: 'right'}}>
                        {this.state.loading ? <h3 style={{display: "inline-block"}}>Loading....</h3> : " "}
                        <Button clicked={this.addMaterialHandler}>Create New Quotation</Button>                    </div>
                </div>
                <div className="container-fliud">
                    <table className="table table-hover table-striped">
                        ...
                        <tfoot className="bg-blue"><tr><td /><td /><td>''</td><td /><td /></tr></tfoot>
                    </table> 
                </div>
            </div>
        )
            ...
            <div className="container-fluid">
                <Modal show={this.state.edit} modalClosed={this.modalCancelHandler}>
                    ...
                    <div >
                        ...
                        <Button clicked={this.deleteMaterial}>Delete</Button>
                    </div>
                </Modal>
                <div className=" ">
                    {content}
