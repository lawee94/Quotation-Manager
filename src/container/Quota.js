...
class Quota extends Component {
  ...
  state = {
    first: {
      ...
      unit: func.Input(
        ...
        Array(100)
          .fill()
          .map((x, i) => ({ value: i, displayValue: i }))
      ),
    },
    second: {
    ...
    client: {
      name: func.Input("input", "text", "Client Name", {
        required: true,
        minLength: 3,
      }),
      compName: func.Input("input", "text", "Company Name", {
        required: true,
        minLength: 3,
      }),
      address: func.Input("input", "text", "Address", {
        required: true,
        minLength: 7,
      }),
  ...
  componentDidMount() {
    ...
    const url =
      localStorage.getItem("choice") === "computer"
        ? "/materials/computer"
  ...
  hydrateStateFromLocalstorage = () => {
    ...
    if (client && mat) {
      ...
      func.getArrayFromState(this.state.client).map((res, index) => {
        let updatedState = func.inputHandler(
          "",
          res.id,
          this.state.client,
          clientDet[index]
  ...
  submitHandler = (event, myState, type) => {
    ...
    func
      ...
      .map(
        (formElement) =>
          (formdata[formElement.id] = myState[formElement.id].value)
      );
  ...
  continueHandler = (event) => {
    ...
    func.getArrayFromState(this.state.client).map((val) => {
      ...
      !func.checkValidity(val.config.value, val.config.validation)
        ? (isValid = false)
        : console.log();
      return "";
    });
  ...
  box = (myState, type, disp, sb) => (
    <form
      className="Inp"
      onSubmit={(event) => this.submitHandler(event, myState, type)}
    >
      {func.getArrayFromState(myState).map((formElement) => {
        return (
          <Input
            ...
            display={disp}
            changed={(event) =>
              this.inputHandler(event, formElement.id, myState, type)
            }
          />
        );
      })}
      <div style={{ width: "100%", display: sb }}>
        <button className="btn btn-green mt-3">Add</button>
      </div>
    </form>
  );
    ...
    const tableRow = this.props.selMat.map((list, index) => {
      return (
        <tr
          ...
          key={index}
          onClick={() => this.props.deleteMat(list.name)}
        >
          <td>{index + 1}</td>
          <td>{list.name}</td>
          <td>{list.unit}</td>
    ...
    let table = (
      <table className="table table-hover" style={{ overflow: "scroll" }}>
        ...
        <tfoot className="bg-blue">
          <tr>
            <td colSpan="5" className="text-white text-right pr-3">
    ...
    return (
      <div className="mb-5">
        <div className="row  my-5">
          ...
          <div className="col-lg-5 bg-white p-3">{box2}</div>
        </div>
        {table}

        <div className="mt-5 mb-5">{box3}</div>
        <button onClick={this.resetHandler} className="btn btn-green mr-3">
          Reset
        </button>
        <button onClick={this.continueHandler} className="btn btn-green">
          Continue
        </button>
      </div>
    );
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    hydrateMaterial: (data) =>
      dispatch({ type: "HydrateMaterial", material: data }),
    deleteMat: (name) => dispatch({ type: "deleteMat", name: name }),
    reset: () => dispatch({ type: "reset" }),
    addClient: (formdata) =>
      dispatch({ type: "AddClient", clientDetail: formdata }),
