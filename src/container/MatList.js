...
class MatList extends Component {
    ...
    loadMaterial = (url) => {
        ...
        query.once('value', snapshot => {
            let mat = []
            snapshot.forEach( val => { mat.push({ key: val.key, value: val.val() }) })
    ...
    updateMaterial = () => {
        ...
        if(isValid){
            let url  = this.state.choiceElement.choice.value  === 'computer' ?
            '/materials/computer' : '/materials/houseWiring'
            Firebase.database().ref(url).child(this.state.key)
            .update({ 'name': this.state.updateElement.name.value, 'price': this.state.updateElement.price.value})
            let index = this.state.materials.findIndex(res =>  res.key === this.state.key )
            this.setState((state) => {
               state.materials[index] = { key: this.state.key, value:{ name: this.state.updateElement.name.value, 
    ...
    deleteMaterial = () => {
        ...
        let updatedState = this.state.materials.filter(res =>  res.key !== this.state.key )
        this.setState({ materials: updatedState })
        this.modalCancelHandler()
    }

    addMat = (myState, types, choice ) => {
        const clickMat = func.getArrayFromState(myState).map( (formElement, index) => {
            return <Input 
                key={formElement.id}
                ...
                changed={(event) => this.inputHandler(event, formElement.id, myState, types, choice )}
                /> 
        })
        return clickMat
    }
    ...
    inputHandler = (event, id, myState, types, choice) => {
        ...
        if(choice){
            updatedState[id].value === 'computer' ? 
                this.loadMaterial('/materials/computer') : this.loadMaterial('/materials/houseWiring')
        }else{
    ...
    addMaterial = () => {
        let url =  this.state.choiceElement.choice.value === 'computer' ? 
         ...
         let values = {name: this.state.inputElement.name.value, price: this.state.inputElement.price.value}
    ...
    showModalHandler = (list, key) => {
        let updated = null
        func.getArrayFromState(this.state.updateElement).map( (formElement, index) => {
            updated = func.inputHandler('', formElement.id, this.state.updateElement, list[index])
            return ''
        })
    ...
    render(){
        let lists = this.state.materials.map((list, index) => {
            return <tr style={{cursor: 'pointer'}} key ={index } 
                        onClick={() => this.showModalHandler([list.value.name, list.value.price], list.key)} >
                        <td>{index + 1}</td>
            ...
            <div> 
                <div className="col-lg-12 mt-4">
                    ...
                    <div style={{float: 'right'}}>
                        ...
                        <div className="mybtn">
                            <Button clicked={this.addMaterialHandler}>Add</Button>
                        </div>
                ...
                <div className="container-fliud">
                    <table className="table table-hover table-striped">
                        ...
                        <tfoot className="bg-blue"><tr><td colSpan="3">" "</td></tr></tfoot>
                    </table> 
                </div>
        ...
        if (this.state.matLoading) { content = <Spinner /> }
            ...
            <div className="container-fliud">
                <Modal show={this.state.edit} modalClosed={this.modalCancelHandler}>
                    ...
                    <div >
                        ...
                        <button className="btn btn-danger" onClick={this.deleteMaterial}>Delete</button>
                    </div>
                </Modal>
                <div>
                    {content}
